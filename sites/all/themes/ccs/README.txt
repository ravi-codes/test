...
ccs was originally developed for internal use to develop themes at Raincity Studios (Vancouver)
After using the ZEN theme for years, we realised that it was getting too complicated, had too much
files and overrides, so we decided to develop a strip down version of it and ccs
was created.
...
ccs is not intended for beginners, and if you're not sure, try ZEN first, and maybe later
try ccs.
ccs is now used for professional projects by multiple drupal agencies around the world.

__________________________________________________________________________________________

Installation

- Download ccs from http://drupal.org/project/ccs
- Unpack the downloaded file and place the ccs folder in your Drupal installation under 
...
- Log in as an administrator on your Drupal site and go to 
  Administer > Site building > Themes (admin/build/themes) and make ccs the default theme.

- if you want to change the name of the theme from 'ccs' to another name like 'mytheme',
follow these steps (to do BEFORE enabling the theme) :

	- rename the theme folder to 'mytheme'
	- rename ccs.info to mytheme.info
	- Edit ccs.info and change the name, description, projet (can be deleted)
	- In ccs.info replace [ccs_block_editing] and [ccs_rebuild_registry]
	  by [mytheme_block_editing] and [mytheme_rebuild_registry]
	- In template.php change each iteration of 'ccs' to 'mytheme'
	- In theme-settings.php change each iteration of 'ccs' to 'mytheme'
...
- ccs.info => provide informations about the theme, like regions, css, settings, js ...
...
The layout used in ccs is fairly similar to the Holy Grail method. It has been tested on 
...
This is how the page template is buit in ccs, and it works in fluid and fixed layout.
...
Once you start using ccs, you will massively change it until a point where it has nothing
to do with ccs anymore. Unlike ZEN, ccs is not intended to be use as a base theme for a 
sub-theme (even though it is possible to do so). Because of this, it is not necessary to
update your theme when a new version of ccs comes out. Always see ccs as a STARTER, and 
as soon as you start using it, it is not ccs anymore, but your own theme.

If you didn't rename your theme, but you don't want to be notified when ccs has a new version
by the update module, simply delete "project = "ccs" in ccs.info

__________________________________________________________________________________________

Thanks for using ccs, and remember to use the issue queue in drupal.org for any question
or bug report:

http://drupal.org/project/issues/ccs
